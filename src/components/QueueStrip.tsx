          ...
          {reps.map((rep, i) => {
            ...
            const chip = (
              <button
                type="button"
                className={cn(
                  'group relative flex flex-col items-center rounded-xl border px-3 py-2.5 transition-all min-w-[128px]',
              ...
              >
                ...
                {tagCount > 0 && (
                  <span className="absolute -top-2 right-1 rounded-full bg-violet-100 text-violet-700 border border-violet-200 px-1.5 py-0.5 text-[9px] font-semibold inline-flex items-center gap-0.5" title={rep.tag_rules.join(', ')}>
                ...
                <div className="mt-1.5 text-center leading-tight">
                  <div className="text-[12px] font-medium text-ink-900 truncate max-w-[110px]">
                    {rep.name.split(' ')[0]}
                  </div>
